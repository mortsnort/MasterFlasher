...
David Allison edited this page on Apr 18, 2024 · 64 revisions
API Documentation
...
List of Third Party Apps which integrate with AnkiDroid
...
Android should prompt the user whether or not they want to grant your app the READ_WRITE_DATABASE permission
Choose "deny"
...
ACTION_SEND intent
The ACTION_SEND intent is a universal intent for sharing data with other apps in Android. AnkiDroid expects the front text to be in the subject, and the back text to be in the main content. Use ShareCompat to build the intent so that information about your app is automatically sent to AnkiDroid with the intent:
...
Sync can also be triggered when your device is connected to your computer via USB, or a terminal emulator that has the am tool from Android Debug Bridge such as Termux:
